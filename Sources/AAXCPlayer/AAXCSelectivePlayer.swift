...
public class AAXCSelectivePlayer {
    ...
    /// Legacy initializer with Data (kept for backward compatibility)
    ...
    public func convertToM4A() throws -> Data {
        // If we have a file handle, use streaming conversion
        if inputFileHandle != nil {
            // Create a temporary file for output
            let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString + ".m4a")
            try convertToM4AStreaming(outputPath: tempURL.path)
            
            // Read the result back into memory for backward compatibility
        ...
        // Legacy path: use in-memory conversion
    ...
    /// Convert AAXC to M4A file using streaming (recommended for large files)
    public func convertToM4AStreaming(outputPath: String) throws {
        guard let inputHandle = inputFileHandle else {
            // Fall back to in-memory conversion if no file handle
            let data = try convertToM4A()
